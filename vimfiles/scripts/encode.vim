...
if !has('gui_running') && g:is_Windows
    ...
    "内部エンコーディングをcp932にする
    if filereadable(g:CFGHOME . '/utf-8-console')
        set encoding=utf-8
    else
        "強制的にcp932に変更
        set encoding=cp932
    endif
elseif filereadable(g:CFGHOME . '/cp932')
    set encoding=cp932
elseif filereadable(g:CFGHOME . '/utf-8')
    set encoding=utf-8
else
    "デフォルト
    set encoding=utf-8
...
function! Let__EnvVar__(cmd)
    let cmd = 'let ' . a:cmd
    if g:is_Windows && has('iconv') && &enc != 'cp932'
